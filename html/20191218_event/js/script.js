...
var $realWidth = window.innerWidth; //모니터 화면의 사이즈가 아닌 브라우저의 실제 가로값을 가져옴
console.log("브라우저 가로폭 : "+$realWidth);

var $realHeight = window.innerHeight;//모니터 화면의 사이즈가 아닌 브라우저의 실제 세로값을 가져옴
...
// 	}
// }
// resizeEvt(); //최초 브라우저 로딩시 이벤트를 발생시켜 현재 사이에 해당하는 항목을 실행하도록 설정



var $head_bg = document.getElementById("header_part");
function scrollEvt(){
	var $scrollY = window.scrollY;
	console.log($scrollY); //브라우저 상단으로부터 세로축으로 얼마나 떨어져 있는가를 값을 가져옴(스크롤 바의 상단 기준)
	if($scrollY >=72){
...
scrollEvt(); //최초 브라우저 로딩시 함수문을 로드하여 현재 스크롤 위치를 파악하고 해당하는 항목을 실행해라 라는 명령
...
function resizeEvt(){
	changeSize(); //해상도가 조정이 되었을 때 함수문을 실행하라는 호출문
}
