...
function ch_img($img, $txt){
	document.write("<img src='"+$img+"' alt='"+$txt+"'>", "<br>")
}
ch_img("img/ace.png", "카드 앞면");
ch_img("img/card.png", "카드 뒷면");
...
function nextImg(){
	...
	if(n >= $arr_pic.length-1){    //n>=3
		n = 0;  //클릭이라는 행동을 취한다면 다음번 클릭했을 때, n=0 가져와야 "img/pic-1.jpg"를 호출할 수 있기 때문에 조건문을 구성
...
// 		n = -1;   //다음번에 클릭 했을 때 n=0 값으로 가져와서 배열 데이터의 첫번째 인덱스로 접근해서 호출해야하기 때문
// 	}
}
...
/*return 문 : 함수 구문 내부에서 진행을 중단하고 더 이상 진행하지 않도록 함수구문 에서 빠져나오는 역할
(반복문에서 break와 동일)*/
function testTxt($name, $ragion){
	document.write("성명 : " +$name, "<br>");
	return; //함수 구문 내부에서는 상단의 로딩 파트까지만 출력
	document.write("지역 : " +$ragion, "<br>");
}
testTxt("정지용", "서울");
...
//전역변수와 지역변수//
//전역변수는 함수구문 외부에서 선언하여 전체(함수구문 내부 및 외부 전체)에 영향을 주는 변수
//지역변수는 함수구문 내부에서 선언하여 함수 내부에서만 영향을 주는 변수(지역변수의 값을 외부로 전달하기 위해서는 반드시 return 참조 변수가 필요)

var $num_01 = 400; //전역 변수
function varNum(){
	var $num_01 = 600; //지역 변수
	document.write("지역변수의 값은 "+$num_01, "<br>"); //지역변수에 대한 값을 출력 600
}
varNum();
document.write("최종 출력 값은 "+$num_01, "<br>"); //전역변수에 대한 값을 출력 400

//전역변수와 지역 변수를 활용한 이미지 넣기
