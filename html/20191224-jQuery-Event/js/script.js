$(document).ready(function(){
	...
	//load() 이벤트 : 페이지가 로딩된 후에 이벤트를 발생시킨다. 외부파일을 호출하여 로드할 때 사용 가능(공통적인 요소인 header와 footer를 호출하여 화면상에 html문서 형태로 가져올 수 있음. 단, 호스팅 서버를 활용하는 것이 필수항목!!!)
...
/*
	...
	#2. 매개변수(e)를 선언하고 바로 하단에 e.preventDefault(); 작성
	#3. a 태그 내부에 href 속성 값으로 javascript:void(0); 작성
*/	

	//클래스의 유무를 판단해서 클래스가 있으면 제거, 없다면 추가
	$("#detail").click(function(){
		var $has_class = $("#detail").hasClass("active");  //클래스가 존재하면 true, 없다면 false
...
*/
	//hover() 이벤트 : 마우스가 영역 내부로 진입했을 때 이벤트 발생+마우스가 영역 밖으로 빠져나왔을 때 이벤트 발생
	$(".card_img").hover(function(){  //마우스가 영역 내부로 진입시 이벤트가 발생(mouseenter)
		$(".card_img").attr("src", "img/card.png");
...
//			alert("입력값 없음");
	...
	$(".index_method input").change(function(){
		...
		$(".hash_group").append("<a href=''>#"+$index_val+"</a>&nbsp;");

		$(this).attr("disabled", "disabled");
	})

	// #1 show() =>display:black / hide() => display:none;
